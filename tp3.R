...
# On arrÃªte la classification lorsqu'on a 3 classes
while(nrow(tab_dist)>3){
  # On trouve les points qui ont la distance min1male
  min_dist<-which(tab_dist==min(C),arr.ind = T)
  min1=min_dist[1,1]
  min2=min_dist[1,2]
  #On calcule les coordonnÃ©es du barycentre de la nouvelle classe
  mat[min1,1]<-(mat[min1,1]+mat[min2,1])/2
  mat[min1,2]<-(mat[min1,2]+mat[min2,2])/2
  # On met à jour la matrice des distances pour tenir compte de cette classe et supprimer les précédentes
...
# Cette fonction sert à définir des couleurs pour rendre visibles les 3 classes de points
id<-t(id)
z<-c(1:300)
for (i in 1:nrow(id)){
  if(id[i,1]==1){
...
# On réalise ici l'affichage des classes
